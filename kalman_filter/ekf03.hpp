...
namespace rbl
{
    ...
    public:
        ...
        void Update()
        {
            ...
            {
                Eigen::Matrix<double,3,1> judge = last_R.transpose() * v_inloop.matrix();
                LOG(INFO)<<"vinloop: "<<v_inloop(0,0)<<","<<v_inloop(1,0)<<","<<v_inloop(2,0)<<", jude: "<<judge(0,0)<<","<<judge(1,0)<<","<<judge(2,0);
                Eigen::AngleAxisd ana(last_R);
                LOG(INFO)<<"angle: "<<ana.angle()<<", axis: "<<ana.axis().transpose();
                LOG_IF(INFO,judge(0,0)<0)<<"less";
                judge(0,0) = std::max(judge(0,0),0.0);

                v_inloop = last_R * judge;
                last_R = R_inloop;


            }
            // p_inloop(2,0)=std::max(p_inloop(2,0),0.0);
            // p_inloop(2,0) = std::min(p_inloop(2,0),1.0);
            ...
            //     auto cost = p_merror.transpose() *  C_R_.inverse() * p_merror + dYY.transpose() * P_pred.inverse() * dYY;
            //     return cost.value();
            // };
            ...
            //     Eigen::Matrix<Scalar, 15, 1> grad = 2 * H_.transpose() *  C_R_.inverse() * Hx_z + 2 * P_pred.inverse() * x_x0;
            //     return grad;
            // };
